...
class TestUncomparedFilesFlag:
    """Verify --uncompared-files inline|separate|none flag."""

    def test_uncompared_default_inline(self, tmp_path):
        """Default behavior includes uncompared files inline."""
        d_a = tmp_path / "a"; d_a.mkdir()
        d_b = tmp_path / "b"; d_b.mkdir()
    ...
    def test_uncompared_none_excludes(self, tmp_path):
        """--uncompared-files none excludes unmatched file lists."""
        d_a = tmp_path / "a"; d_a.mkdir()
        d_b = tmp_path / "b"; d_b.mkdir()
        (d_a / "only_a.py").write_text("x = 1\n", encoding="utf-8")
        json_path = str(tmp_path / "out.json")
    ...
    def test_uncompared_separate_creates_file(self, tmp_path):
        """--uncompared-files separate creates a standalone text file."""
        d_a = tmp_path / "a"; d_a.mkdir()
        d_b = tmp_path / "b"; d_b.mkdir()
        (d_a / "only_a.py").write_text("x = 1\n", encoding="utf-8")
        json_path = str(tmp_path / "out.json")
        main([
            str(d_a), str(d_b),
            "--report-json", json_path,
            "--uncompared-files", "separate",
        ])
        from pathlib import Path
        sep_path = Path(json_path).with_name("out_uncompared.txt")
        assert sep_path.exists()
        content = sep_path.read_text(encoding="utf-8")
        assert "only_a.py" in content
