...
## 구현 진행 총괄 (2026-03-15 14:30 기준)

> **전체 진행**: 9개 항목 중 **8개 완료**, 테스트 **304 passed** / 4 skipped / 10 xfailed  
> **브랜치**: `dev` (main 미반영)

### 완료 항목

| §  | 항목 | Commit | 핵심 변경 | 검증 결과 |
|:--:|------|--------|----------|----------|
| 1  | AFC 점수 통합 | `b8f3164` | `_AFC_SSO_DECL_MIN=0.75`, `afc_filtered` kwarg | Guava:Lists FP 제거, xfail 2건→pass |
| 2  | 도메인별 프로파일 | `c0576e` | `_CLASSIFICATION_PROFILES` (industrial/academic), `profile` kwarg | neg_raw_max=0.62 기반 academic 임계값 |
| 3  | 2단계 분류 체계 | `922fad1` | `_classify_strict()` + `_classify_relaxed()` | **Recall +22.5%** (24.3→46.9%), SUSPICIOUS precision 76.9% |
| 4  | Normalized 채널 | `89aae3c` | `_SSO_NORM_RAW_RATIO=1.2` defensive guard | 양성 median ratio=1.44, 현행 FP/FN 변화 0 |
| 5  | Composite 가중치 | `a61b009` | ROC AUC 비례 가중치, `declaration_cosine` 활성화 (0→0.58) | 회귀 0 |
| 9  | 법리 델타 분석기 | `(latest)` | `analyze_legal_defense_pattern()` 5패턴 + 4 헬퍼 함수 | 9 TDD 테스트, CleanRoom/Merger/Factory 검증 |
| —  | xpass 정리 | `b8f3164` | 2 xfail markers 제거 | 10 xfails 잔존 (§2/§7 타겟) |
| 8  | Comment 채널 | `1eaba31` | license filter, Javadoc stopwords, `comment_string_overlap_tfidf()` | AUC=0.8469, 회귀 0 |

### 변경 파일 요약

| 파일 | 변경 유형 | 설명 |
|------|:--------:|------|
| `src/diffinite/evidence.py` | 수정 | 6개 기능 추가 (AFC 임계값, 프로파일, 2단계, norm/raw, 가중치, 법리 분석기) |
| `TDD/corpus/test_domain_convergence.py` | 수정 | xfail 2건 제거 |
| `TDD/legal_defense/*.java` (6파일) | 신규 | 법리 방어 테스트 Java 쌍 (MergeSort, Singleton, Factory) |
| `TDD/legal_defense/test_legal_defense.py` | 신규 | 9개 법리 방어 TDD 테스트 |
| `TDD/corpus/afc_score_analysis.py` | 신규 | AFC 점수 인플레이션 분석 |
| `TDD/corpus/norm_raw_analysis.py` | 신규 | norm/raw 비율 분석 |
| `TDD/corpus/norm_ratio_simulation.py` | 신규 | SSO 규칙 norm 조건 시뮬레이션 |
| `TDD/corpus/suspicious_metrics.py` | 신규 | SUSPICIOUS 리콜 회복 측정 |
| `TDD/corpus/domain_profile_analysis.py` | 신규 | 도메인별 점수 분포 분석 |

### 미완료 항목

| § | 항목 | 상태 | 비고 |
|:-:|------|:----:|------|
| 7 | 코퍼스 확장 (SOCO test set 등) | 보류 | 사용자 지시로 나중에 진행 |
...
**Step 1–2 구현**: `evidence.py`에 필터링 인프라 추가 (commit `1eaba31`)

```python
# 라이선스 헤더 필터
_LICENSE_KEYWORDS = {"copyright", "license", "apache", "gpl", "mit", ...}

# Javadoc 태그 stopword
_JAVADOC_TAGS = {"@param", "@return", "@throws", "@see", "@author", ...}
```

- `extract_comments_and_strings()`에 `filter_license=True`, `filter_javadoc_tags=True` 기본 활성화
- `_is_license_line()`: 라이선스/저작권 줄 자동 감지 및 제거
- `_strip_javadoc_tags()`: Javadoc 태그 마커 제거 (내용은 보존)

**Step 3 구현**: `comment_string_overlap_tfidf()` 신규 함수
- `_IDENT_RE` 토크나이저 + `_tfidf_vector()` + `_cosine_from_counters()` 패턴 재사용
- IDF 없으면 기존 Jaccard `comment_string_overlap()`으로 자동 폴백

**AUC 측정 결과**: 현재 코퍼스에서 `comment_string_overlap` AUC = **0.8469**
- TODO 원본 수치 0.528은 초기 측정값 — 8-Stage 파이프라인 개선 과정에서 이미 크게 향상됨
- TF-IDF 버전은 AFC 파이프라인에서 IDF 사전이 제공될 때 Jaccard를 완전 대체 예정

**회귀 테스트**: 304 passed, 4 skipped, 10 xfailed = **0 regressions**

**Step 4–6 (후속)**: TF-IDF 버전 ROC AUC 비교, 가중치 재조정은 코퍼스 확장(§7) 후 진행
